class Sql {
  ...
  generateQuery() {
    ...
    if (this.mainMenu == 'Update an employee') {
      return `UPDATE employee SET role_id = ?, manager_id = ? WHERE id = ?`
    }
    if (this.mainMenu == 'View payroll by department') {
      return `
        SELECT D.name AS department, SUM(C.salary) AS Payroll
        FROM employee A
